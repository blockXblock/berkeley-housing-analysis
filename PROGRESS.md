...
**SWOOP (John's spec):** every corridor tour opens 50 m behind the start at 75 m above cruise, holds **8 s** facing 150 m down the route (tilt 69.4°), descends 4 s onto the start; at the end climbs 50 m beyond, turns to the **reciprocal heading** and holds 8 s looking back. `scripts/add_intro_swoop.py`, marker-guarded and idempotent. Applied to all 21 generated corridor tours, orbit and cruise. **24 s of swoop is 3.5% of Ashby's 680 s but 20% of Adeline's 119 s** — watch Adeline for front-loading.
**`--orbit-at lat,lon,alt,radius[,label]` ADDED** to `gen_corridor_tour.py`: `--orbit` matches names in geometry.kml and there are **ZERO buildings west of I-80**, so the yacht harbour (and any park, pier, intersection or the Campanile) could not be orbited at all. Alt and radius are what you want to see; the flag backs out the internal `roof+10` and `rad*2.2`. Verified over open water.
**⚠ EAz7HmY0y-Q (University, the FIRST video on the homepage) NEEDS RE-RECORDING.** My west-end extension prepended three vertices in EAST-TO-WEST order, so the flight opened by running 490 m backwards then reversing — a 180° bend in a hand-drawn path. Fixed (bearings now 88/88/88/76/76/81/81/81, max bend 12.1°, true length 3.88 km not the 4.37 I reported). **John saw the U-turn, thought it charming, and left it in;** he now wants University to start WEST of I-80 instead, so that end is being redrawn anyway.
**PLANNED — a Bay tour:** start on the marina pier, orbit the yacht harbour (`--orbit-at`), end at the I-80 overpass at `-122.3095`, which is exactly where University begins, so the two cut together as one flight from pier to campus gate. **Joining two tours = concatenating their `<gx:Playlist>` elements into ONE `<gx:Tour>`; Earth has NO command to chain tours** and Movie Maker stops between them. `join_tours.py` deferred until the real pair exists. The seam needs the inner swoops stripped or the camera whips 180° between two 8-second holds.
...
**2036 BANCROFT REFIT — gated write DONE (snapshot `scratch/2026-08-26/geometry_pre-2036-bancroft-refit.kml`).** John spotted the footprint sitting inside an existing building. It was: a 11,636 sf box floating in the INTERIOR of APN 055-1894-017-04, **19 m short of Bancroft and 8 m short of Shattuck — touching no street.** The 2025-10-02 RESUB Entitlement Plan Set (local, `scratch/2026-08-23/harvest_stage_batch2/20/`) is text-native and settles it: **the project is a LOT LINE ADJUSTMENT.** APN -016 grows 6,595 -> 14,582 sf, -017-04 shrinks 37,696 -> 29,709 sf; **7,987 sf transfers, both sides matching to the square foot.** The 8-storey / 85 ft tower sits on the ENLARGED -016 at 80% coverage with a stated **FRONT YARD SETBACK 0'-0"**. Derived: 30.8 m of Bancroft frontage (13.7 existing + 17.0 taken) x 35.0 m deep = 11,608 sf vs the stated 11,610. Written; **north face verified 0.00-0.05 m off the Bancroft property line**, SW corner in -016 and SE in the transferred strip. -017-04 keeps the existing 4-storey 2300 Shattuck building (31,115 -> 24,535 sf, height NO CHANGE).
**Units 85 -> 87, PRIMARY-SOURCED, not from APR Table A.** The plan set's own unit table reads 31 studio + 8 one-bed + 13 two-bed + 35 three-bed = **87**, and a sheet states "TOTAL UNITS 87 UNITS" (8 below-market-rate). Table A agrees at 87 but is the verification target, never the input — the figure stands on the plan set alone. Label and description body both updated (they disagreed at 85/85 before only by luck; see the four survivors below).
**53 packages rebuilt at `geom-c4284bfe1833`, catalog repointed + verified (11 entries, 0 missing), `docs/geometry.kml` republished in sync.** **Pruned the 53 superseded `geom-a8f468e28e3c` packages** — one generation on disk is the standing convention (verified across the last 8 commits), but **`build_tour_package.py --all` does NOT prune**, so every geometry edit leaves a full stale generation behind. Worth a guard next to the catalog one.

**FOUND, HELD FOR JOHN — 2920 Shattuck (proj8) is wrong in three ways, all settled by its 1.E** (`documents` 2162, Tabulation Form dated 09/22/2025, fetched from R2):
- **Footprint 5.0 m west of the Shattuck property line** (measured against assessor parcel 53-1590-5-1's frontage). The 1.E states **FRONT YARD SETBACK "VARIES; 0 to 2'"** — the east face belongs ON the line. The polygon was synthesised as a box from the AREA only (lot 19,586 x 52%), never placed from a site plan.
- **HEIGHT UNDER by 3.2 m.** 1.E says **11 stories, 125'-2" average AND maximum = 38.2 m**; geometry carries 10 storeys / 35.0 m. Feet beat storeys x 3.5 (the standing rule), so 38.2 m. **This is a Shattuck-corridor building the tours orbit** (one of the 6 orbits in `shattuck-s2n-path.kml`, orbit altitude = roof + 10 m), so the orbit re-cuts when it lands.
- **Description body says 221 units against a 242 label**; the 1.E says **242** proposed dwelling units, so the label is right and the body is stale.
- **Four label-vs-body unit disagreements survive the eadd312 sweep**: 1950 Oxford (772/300), 2920 Shattuck (242/221), 1752 Shattuck (72/68), 2733 San Pablo (152/32). 1950 Oxford is the UC beds-not-units case.

**`docs/tours/Control Points/` REMOVED** (John: everything belongs in `kml/`). Audited before deleting, not after: nothing in `docs/*.html|js|json` or any script referenced the folder; the three San Pablo files were byte-equivalent to `kml/tours/control_points/San Pablo Control Points DERIVED.kml` (10 points, same first/last coordinate, three formats of one thing); `Shattuck-S-N.kml`/`.kmz` matched the path as it stood BEFORE John's pin 04 move, so they were stale, not duplicates — a copy of the corridor that would have flown a different line. The folder's `transparent-1x1.png` was the icon's original home, but the scripts now source it from `kml/tours/labels/`, and copies exist wherever a KML references it (`kml/tours/labels/`, `kml/geometry/`, `kml/tours/packages/`, `docs/`). Folder is gone; `docs/tours/README.md` and `docs/tours/Elmwood-Downtown/` are untouched.

**ALL THREE PUBLIC KML DOWNLOAD LINKS NOW POINT AT `docs/geometry.kmz`** (`index.html` "Open KML in Google Earth", plus the "Download Berkeley Skyline KML" links on `explorer.html` and `explorer_v2.html`). The bare `.kml` would have handed every downloader the same failure John hit: its 180 label styles reference `transparent-1x1.png` by RELATIVE href, and Earth falls back to a network fetch it cannot satisfy. The `.kmz` carries the image inside — verified self-contained, 196 placemarks, byte-identical doc.kml to the canonical, and 35 KB against the .kml's 299 KB, so it is also an 8.6x smaller download. `docs/geometry.kml` stays published as the readable/diffable copy; only the LINKS moved. `tours.json`'s `canonical_geometry.file` still names the `kml/` SOURCE, which is correct and was left alone.

**CORRIDOR SET COMPLETED + STREET SIGNS FOR ALL OF THEM (John's request).** Packages now **57** at `geom-b73b0ae495c8`.
- **Ashby and Durant did not exist** — `ashby-bart` is a BUILDING LOOP, not an Ashby Ave corridor, and there was no Durant tour at all. Both now derived and built (Ashby 34 cp / 114 wp / 6.0 min; Durant 13 cp / 40 wp / 2.0 min).
- **NEW `scripts/derive_corridor_from_addresses.py`** — the cheap general derivation: addresses sit on BOTH sides of a street, so the MEDIAN address position per slice tracks the roadway. Complements `derive_street_centreline.py` (parcel-gap, more accurate, needs a hand-tuned corridor entry). **`--axis` is load-bearing** — a N-S street slices by latitude for a median LONGITUDE, an E-W street is the transpose; slicing the wrong way returns one meaningless point. Output is CANDIDATE geometry, same caveat as the parcel-gap method.
- **COLLEGE-BANCROFT L-ROUTE built to John's spec** — College Ave S->N starting at **37.8455, ~400 m south of the Berkeley/Oakland line** (Berkeley's address data stops at the city line, so the southern start is EXTRAPOLATED along the local bearing), north to Bancroft, then **1,203 m west along Bancroft to Shattuck**, ending on the Shattuck path's own longitude so the two corridors meet. 23 control points -> 97 waypoints / 5.5 min. Generator: `scratch/2026-08-26/build_college_bancroft_path.py` (one-off); durable artifact is `kml/tours/control_points/College-Bancroft Path S-N-W.kml`.
- **`gen_street_labels.py` READ ONLY LineString CONTROL POINTS**, so six of the seven corridors could never have had street signs — everything `derive_street_centreline.py` emits is a folder of numbered Point placemarks. Now accepts both shapes, ordered by pin NAME. Signs generated for all eight corridors (Ashby 24 cross streets, Bancroft 21, Telegraph 15, San Pablo 15, College 13, University 11, Adeline 6, Durant 6).
- **NEW `scripts/retime_tour.py`** — the hand-made tours (200+ units, the dramatic cuts) were not generated by `gen_corridor_tour.py` and have no `--speed`, so it scales `gx:duration` instead. **200+ unit tour slowed x1.5: 8.8 -> 13.2 min**, 211 waypoints byte-identical in position, heading and tilt (asserted, not assumed). Its guard caught a real bug in itself first: the tour has **211 Camera blocks but only 210 `<roll>` tags**, so a `<longitude>...</roll>` comparison ran past the roll-less camera into the next leg.

**⚠ THE SPLIT SHIPPED A COLOUR REGRESSION FIRST — John caught it: "why have the colors of the projects in review turned grey?"** The splitter twinned every `<Style>` as `<style>_nolabel` but **NOT the 83 `<StyleMap>`s**, and 83 of the 196 placemarks reference a StyleMap rather than a Style. So **80 polygon placemarks named a `#..._nolabel` that did not exist**, and Earth silently fell back to its DEFAULT white/grey fill. **A dangling `styleUrl` raises no error in KML** — it just renders default, which is why every verification I ran (well-formed, 196 extrudes, 0 coplanar anchors, consumers still parse, round-trip identical) came back clean while the skyline had lost its status colours. **I verified structure and never verified APPEARANCE.**
**Fixed + gated:** the splitter now emits a `_nolabel` StyleMap twin whose pairs point at the `_nolabel` Style twins, and **REFUSES TO WRITE a file containing any styleUrl that resolves to nothing** (`SystemExit`, naming the offenders). New check added to the verification set: resolve every polygon's PolyStyle+LineStyle colour through its StyleMap and assert it is UNCHANGED from the pre-split file — **0 of 196 changed**. Regenerated at **`geom-b73b0ae495c8`**; 520 styleUrls, 0 dangling.
**LESSON: for a rendering change, structural checks are not sufficient evidence.** Placemark counts, well-formedness and round-trip equality all passed on a file that displayed wrong. Anything that alters how the map LOOKS needs an appearance-level assertion (resolved colour, resolved style) or John's eye — and John's eye found it in one pass.

**LABEL LOD SPLIT — distant label churn fixed structurally (`scripts/split_label_lod.py`).** John: labels still jump when far away, and proposed moving them streetward / down the facade / onto the approach side. **MEASURED FIRST, and none of the three would have worked:** along the Shattuck cruise, **125-150 buildings fall within 2 km of a typical waypoint but only 6 within 300 m** — so at distance ~150 labels fight for one screen and Earth's declutterer re-picks winners every frame. That is the jumping. Streetward would have made it WORSE (every building fronts the same corridor, so the labels collapse onto one line); down-the-facade reintroduces the depth conflict the anchor lift just removed; approach-side helps only OCCLUSION and bakes a direction into geometry shared by every tour, including the N-to-S one.
**THE FIX IS `<Region>`/`<Lod>`** — a label draws only once its building exceeds `minLodPixels` on screen. **A Region governs its whole Placemark**, though, and ours were `MultiGeometry(Point + Polygon)` — so attaching one would have hidden the BUILDINGS at distance and destroyed the skyline. Hence the split: **196 placemarks -> 392**, a polygon twin (name + description + extrusion, on a `_nolabel` style whose LabelStyle scale is 0, NO Region, always drawn) and a label twin (name + lifted Point + Region). **The description is NOT duplicated** — 39 description bodies had already drifted out of step with their labels once here, and two copies of the same prose is how that recurs.
**22 placemarks had no `<Point>` AT ALL** — every BART building, Dharma University, both Innovation Zones, 1717 San Pablo. Earth was drawing their labels at the polygon centroid, unregioned AND never lifted, so they were among the worst offenders. The splitter now synthesises the anchor the other 174 already had.
**Verified:** 392 placemarks = 196 polygon + 196 label; 196 Regions, 0 on a polygon; 196 extrudes intact; 0 polygon placemarks unsuppressed; 0 anchors coplanar; **`--unsplit` round-trips byte-identically**; and BOTH consumers still read 174 buildings unchanged (`gen_corridor_tour` and `gen_building_loop` match on `<b>ADDRESS</b>` + `<Polygon>`, which is why name and description stayed on the polygon twin). **`set_label_scale.py` now SKIPS `_nolabel` styles** — restyling them would have put all 196 labels back on screen twice. Packages rebuilt at **`geom-dd8a95225384`**. Tune the appear-distance with `--min-lod` (128 now; higher = later/closer) without another restructure.

**VIDEO PUBLISHED + LABEL MECHANICS SETTLED.** First recording on the corrected geometry is live: **`fv-R5dLfY5U` — "Shattuck Avenue, South to North — August 2026"**, now the FIRST video on `docs/index.html`, above Patrick Kennedy. The June Shattuck video (`X0IMsbbhjGk`) STAYS — verified it runs 37.882 -> 37.852, i.e. NORTH to south, so the two are different flights, not duplicates.
**`docs/tours.json` gained its 12th entry** — `shattuck-s2n-cruise` was one of the 42 packages the catalog never listed. `recorded_geometry_era` records the generation John actually flew (`geom-0cc83edd59a0`, labels 5.0 with coplanar anchors), NOT the current one, so the flicker is traceable to the recording.
**LABEL FLICKER — DIAGNOSED AND FIXED.** John reported jittery/flickering building labels. **171 of 174 label anchors sat at EXACTLY the roof altitude** — each `<Point>` billboard was depth-tested against the coplanar top face of its own extrusion and flipped in and out as the camera moved. `set_label_scale.py --lift` now raises every anchor by `max(3 m, roof*5%)`; 0 remain coplanar. Second, independent flicker source at large scale is DECLUTTER CHURN (Earth re-deciding which of 196 overlapping labels to draw); John's call: **building labels dropped 5.0 -> 3.0** for the next Movie Maker run. Street signs stay 2.5 / 3.125.
**THE ICON IS WHAT MAKES `scale` WORK** (measured, ladder of 7 settings flown down Shattuck): with `<IconStyle><scale>0</scale>` and no href Earth largely IGNORES `LabelStyle`, and adding one at 1.8 made labels look SMALLER than no LabelStyle at all. All 180 styles now carry a transparent 1x1 PNG at icon scale 0.4. **The href is RELATIVE and Earth may fall back to a NETWORK fetch** (it did) — so `build_tour_package.py` emits a **.kmz twin** of every package with the image zipped inside, plus `docs/geometry.kmz`. The .kmz is the copy to open and to publish; **the site's "Open KML in Google Earth" button still points at the bare .kml and should be repointed before the next deploy.**
**CATALOG SELF-CONTRADICTION FIXED:** `package_geometry_sha` was `5bb87b9b029c` on EVERY entry while the package paths said otherwise — the repoint regex only rewrote `geom-XXXX` in the path, never the bare sha in its own field. A catalog that disagrees with itself is how the sitewide 404 went unnoticed; the guard now repoints both.
**GE VIDEO QUALITY (John: imagery fills in late, 3D low-res).** Not the labels (a few hundred textured quads) and not GPU — GPU starvation lowers the FRAME RATE, whereas detail arriving late is TILE STREAMING. Fixes are Earth-side: pre-fly the tour twice to warm the cache; raise **Preferences -> Cache -> Disk Cache** (Mac path is the app menu, NOT Tools > Options); **3D View -> Anisotropic Filtering High** (grazing-angle textures are the entire frame at tilt 88) + Terrain Quality max; record with **Tools > Movie Maker** from a saved tour, which renders frame-by-frame rather than racing the tile loader. ⚠ Raise the DISK cache, not memory — this Mac has 8 GB and Earth has already OOM'd with the geometry loaded. Tour-side levers if needed: `--speed 8` (slower) or `--cruise 40 --tilt 75` (sharper but loses the in-the-street feel).
**NEXT — RE-RECORD PASS (John's plan):** redo each existing video on the corrected geometry. `X0IMsbbhjGk`, `3OQjzk9dIOw`, `5VLjGlMuHLU` are all catalogued `needs_rerecord: true` (flown May/June, before the 1.E heights, the BART buildings, the dedup and today's corrections). **`2mVF8kdjY-U` (Patrick Kennedy) and `LAW1WIUF_ks` (Adeline-Shattuck) are NOT in the catalog at all** — nothing records which geometry they show. **UPDATE 2026-09-05:** Kennedy re-recorded — `2mVF8kdjY-U` -> **`9t5bdNZwhFA`** on the homepage and in the catalog (commit `4e1e006`). The catalog entry is STILL the `unsourced-` placeholder (era UNKNOWN, `needs_rerecord: true`, no `tour_kml`), so the generation it shows is still unrecorded. `kml/tours/panoramic-kennedy-legacy-slow20.kml` now exists and its package is `geom-167622a198b3`, but current geometry is `geom-a3d103322890` — `publish_video.py` will refuse until the Kennedy package is rebuilt at the current sha, and only John knows which package he actually flew. Give every video a catalog entry naming its generation as part of the pass.

**2036 BANCROFT — POSITION NOW MEASURED, NOT DERIVED (supersedes the earlier refit; snapshot `scratch/2026-08-26/geometry_pre-2036-siteplan.kml`).** John pushed back on "the position is still not known" and was right to. I had stopped at the zoning tabulation and never opened **sheet A100 PROPOSED SITE PLAN (1"=10'-0")**, which dimensions the enlarged parcel outright in its key map: **APN 55-1894-16 = 113'-1" along Bancroft x 130'-0" deep, with 118'-10" from its east edge to Shattuck.** Plan note 1 states the LLA explicitly. Setbacks are dimensioned on the sheet: **0' required east, (P) 7'-0" side setback west**, 5'-0" rear.
**My derived ring had the right AREA and the WRONG SHAPE** — I had assumed the enlarged lot kept -016's 44 m depth and solved for a 30.8 m frontage; the real lot is **34.47 x 39.62 m**. Because the area constraint was satisfied either way, nothing in my own checks could have caught it — only the drawing could. Corrected footprint **32.33 x 33.36 m = 11,608 sf** (stated 11,610), front face on Bancroft (NW 0.00 m / NE 0.09 m off the property line), 6.27 m rear gap against the 5'-0" minimum. **Independent cross-check:** the sheet's 118'-10" (36.2 m) lot-east-to-Shattuck reads 34.2 m against the assessor ring — 2.0 m apart, consistent with the assessor rings' own imprecision (they also disagree with the sheet by ~3 m on total Bancroft frontage).
**LESSON — EXHAUST THE DRAWING BEFORE DERIVING FROM THE TABLE.** A zoning tabulation gives areas; a SITE PLAN gives position, and position is what a skyline needs. The same set had the answer on a sheet I had already listed by name and not opened. Sequence for any placement question: key map / site plan dimensions FIRST, tabulation second, arithmetic last. 53 packages rebuilt at **`geom-4999f40cceb2`**, catalog verified.

**2920 + 2955 SHATTUCK FIXED (gated write, snapshot `scratch/2026-08-26/geometry_pre-2920-2955-fix.kml`).**
- **2920 Shattuck (proj8)** — everything from its own **1.E Tabulation Form dated 09/22/2025** (`documents` 2162, fetched from R2), PROPOSED column. Moved **4.97 m EAST** onto the Shattuck frontage of APN 53-1590-5-1 (the 1.E states FRONT YARD SETBACK "VARIES; 0 to 2'"); the polygon had been synthesised as a box from the AREA alone and floated 5 m behind the sidewalk. **Height 35.0 -> 38.2 m and 10 -> 11 storeys** (form says 11 stories, 125'-2" average AND maximum; feet beat storeys x 3.5). Body units **221 -> 242** (label was already right).
- **2955 Shattuck (proj22)** — **1.84 m EAST**. **My first check measured the wrong edge:** 2955 is on the EAST side of Shattuck, so its frontage is its WEST edge, not its east. The west edge overhung the property line into the roadway. Site is TWO parcels — 53-1589-18-1 (2925 Shattuck, 12,235 sf) + -19 (2921, 4,831 sf) = 17,066 sf vs the 1.E's stated 16,968 sf lot; at **92% coverage** the building nearly fills the site, so any placement error shows up immediately as street intrusion. **Lesson: determine which side of the street a parcel is on before calling an edge the frontage.**
- Both verified at **-0.00 m** from their property lines after the write. **The 2920 height change propagated**: its building loop re-cut (eye 45.0 -> 48.2 m) and the Shattuck orbit tour re-cut (orbit altitude 45.0 -> 48.2 m). Tours regenerated (`08-26 21:06 · cp-f9cc42`), 53 packages rebuilt at **`geom-80719d22b81f`**, catalog verified.
- **⚠ PRUNE BUG, CAUGHT AND FIXED IN-SESSION:** I pruned superseded packages by picking the most FREQUENT filename suffix. With 53 old and 53 new that is a TIE, and it deleted the NEW generation, leaving `docs/tours.json` pointing at 11 files that no longer existed — **re-creating the exact sitewide 404 the catalog guard exists to prevent.** Rebuilt and re-pruned against the **actual current sha read from the catalog**, never frequency. **`build_tour_package.py --all` still does not prune at all** — worth folding a sha-anchored prune into the same guard so this cannot be done by hand again.

**SHATTUCK CONTROL POINTS — pin 04 moved by John, path + both tours + 53 packages REGENERATED.** New tool `scripts/control_points_roundtrip.py` (`--explode` path -> numbered draggable pins, `--rebuild` pins -> path). **WHY:** a hand-drawn GE Path is ONE Placemark holding ONE LineString; its vertices are not objects, so Earth draws a bare line and the vertices exist only as edit handles in edit mode — John saw a white line and no control points, correctly. Exploding to 18 numbered Point placemarks makes them visible and individually draggable. **Order comes from the pin NAME, never file order** (Earth reshuffles placemarks as they are edited). **Round trip is LOSSLESS** — verified explode->rebuild with nothing dragged leaves the path byte-identical, `cp-50574a` unchanged. (First cut used `%.10f`/`%.13f` and silently rewrote the canonical path file with truncated coordinates for a non-change; fixed to Python float `repr`, the shortest exactly-round-tripping form.)
**Applied:** pin 04 (lat 37.86035) +6.41 m E / +10.65 m N. `cp-50574a` -> `cp-f9cc42`; both tours regenerated (`Shattuck S-to-N` 451 wp / 498 s, `(no orbits)` 103 wp / 344 s, both stamped `08-26 20:59 · cp-f9cc42`); 53 packages rebuilt, superseded generation pruned. **25 of 103 cruise waypoints moved, lat 37.85996-37.86860, max 12.4 m** — i.e. only the two segments adjacent to the moved pin, tapering to zero at pins 03 and 05. **A single pin does NOT shift the whole run.**
**MY GAP-CENTRELINE MEASUREMENT IS A PROXY, AND JOHN'S EYE OVERRULES IT.** I had measured pin 04 at -0.6 m from the parcel-gap centre and queried the move; John: the path is west of the street as rendered. The gap method derives the centre from parcel EDGES, which is not the visible roadway centre (asymmetric ROW, sidewalks, varying setbacks) — `derive_street_centreline.py`'s own docstring says the human eye on real imagery is the source and the derived line is CANDIDATE geometry. Treat the -4.5/-5.2/-15.8/-7.8 m readings at pins 05/09/10/12 as hints, not targets.
**GE SAVE BEHAVIOUR (John asked why):** Earth does not edit files in place. Opening a KML parses it into the in-memory Places tree and drops the file connection; **Save Place As is an EXPORT to whatever directory the file dialog last used** — it defaulted to `docs/tours/Control Points/` (which holds older Shattuck/San Pablo control-point copies) rather than the `kml/` source. Every edit is a copy-out that needs steering back. **`docs/tours/Control Points/` holds stale duplicates of `kml/` control points — flagged for consolidation, not deleted.**

**SHATTUCK CONTROL POINTS — John reports the northern flight ~5 m too far west; MEASURED and confirmed.** Each of the 18 hand-placed vertices in `kml/tours/control_points/Shattuck Path S-N.kml` compared against the parcel-gap centreline (the `derive_street_centreline.py` method, run against `berkeley.db` rather than the geojson): the southern half is within ~2 m, but **v4 -4.5 m, v8 -5.2 m, v9 -15.8 m, v11 -7.8 m — all WEST of centre, all in the northern run** (lat 37.869 and 37.880-37.882). Six vertices sit at intersections where the gap method is unusable (gap <8 m or >45 m) and were skipped, so the drift may extend further north than the four measured. Editing route + regeneration chain reported to John; **nothing edited — the path is his hand-drawn source.**
...
**5. Overture heights RESOLVED by primary sources (`521232c`, `eadd312`)** — both disputed Overture-derived heights are now primary-sourced from DRC staff reports, verified firsthand from the PDFs (not taken on the peer's report). **2000 University**: DRCF2022-0001 states "a new 84' tall 8 story mixed use" + "81 studio dwelling units" → Overture's 8 was RIGHT, and the stated 84 ft refines 28.0 → **25.6 m** via feet-beats-storeys. **2009 Addison**: DRCP2017-0001 states "7-story" → 7 confirmed, no stated feet, so **24.5 m unchanged**; provenance only. **No Overture-derived heights remain in the skyline.**
  - **The density check was decisive in NEITHER direction, and both its readings were wrong.** 07 called 2000 University "impossible" at 610 gross sf/u — it is 81 STUDIOS, and the objection conflated gross with net. 07 called 2009 Addison corroborated at 1,510 sf/u — that was over the ceiling only because the check assumed all 7 floors residential; the report says units occupy the **5 upper floors** (ground floor commercial), giving 1,103 sf/u, in band. **LESSON: the per-unit gate must use RESIDENTIAL floors, not total storeys, or every mixed-use building false-flags.** Three inference methods (Overture-as-class, density, storeys×3.5) all failed where one cheap text-native document stated the answer outright.

**6. 39 stale description bodies synced (`eadd312`)** — a placemark's description balloon (what a viewer sees on click) had drifted from its own label: 3030 Telegraph read "Under Construction" for a Completed 144-unit project, 3000 San Pablo read 29 units against a real 78. Labels verified against v2 (exact match) before treating the body as stale. 0 disagreements remain.

**7. Catalog auto-repoint (`c6b622e`)** — `build_tour_package.py --all` now repoints `docs/tours.json` and verifies every entry resolves BEFORE writing, exiting non-zero rather than publishing broken links. Fired correctly on 4 subsequent real rebuilds. **This is what stops John's path edits from re-404-ing the site.**

**OPEN / NEXT:** (a) **65 placeholders remain; the BP-description method cannot reach them** (43 state no floor count, 9 match no v2 project, rest demolition-only). **THE BIG LEVER, sized but not run:** the question is NOT "which have a 1.E" (that lever is spent — only 1 of the 65 has one and it's image-only). 2000 University had NO 1.E, yet its **DRC staff report stated storeys AND feet in clean pdftotext-readable prose**. Staff reports reach exactly the buildings BP-description can't — a Design-Review project has one even when its permit says only "construct ADU". Cheap (~250–380 KB), text-native, explicit. **07 is ready to run a discovery sizing pass across the 65 the moment John greenlights scope.** (b) The catalog lists only 11 of 53 packages — corridor tours + 11 single-building loops are built but unlisted; **the website menu for building loops is still to be designed** (John's stated intent). (c) Still open for John: the 2205/2201 Blake APN fix (gated v2 write, 07 flagged); label colour (tier vs white); canonical set (184 towers vs full 1,060). (d) John draws GE Paths for corridors — derivation agreed to be not worth the effort.

## 2026-08-25 [geometry/1.E/BART — session berkeley-data-07] — 🏗️ 1.E harvest + heights + footprints + BART placeholders — WRAPPED + DEPLOYED
**✅ DEPLOYED to origin/main (`d90540a`, `bcd1e06..d90540a`) — SELECTIVE, John-approved: 110 map+tour files (3 maps w/ Chronicle-clipboard + Regrid link; docs/geometry.kml 196 placemarks; docs/tours.json catalog-repointed = FIXES the tour-download 404s; kml/ tour packages/loops/control-points). berkeley2050/notes/team EXCLUDED (safety-gated). berkeley2050 is INTENTIONALLY public (John, leave it). CF propagation may lag ~4h. Video-record inputs (geometry.kml + 53 kml/tours/packages) ready locally on dev.**
**HEIGHT THREAD CLOSED: every camera-relevant building is primary-sourced. 2000 University 8st/84ft/25.6m + 2009 Addison 7st/24.5m confirmed via DRC STAFF REPORTS (the cheap text-native source — beats 1.E OCR + parcel arithmetic). Overture = SKIP (validate each value; it was right on these two but wrong on 1951 Shattuck). Staff-report sizing pass: only 8/65 no-height placeholders reachable (`data/reference/staff_report_sizing.csv`); the other 57 = ministerial ADUs, no DR record, hold. 2200 Fifth (proj45) calibration PASSED (staff report 3st == BP-description 3st), validating 2b's 22 BP-derived heights. Density gate BUG (2b): use RESIDENTIAL floors not total for mixed-use, treat mixed-use as unresolved-not-failed. Third trap: strip [...] annotations (they quote old wrong values) — guarded in resolve_placeholders.**
**Collaborating with berkeley-data-2b (owns geometry.kml + tour packages) — SESSION ENDED. DIVISION was: HEIGHTS = 2b (1.E stated wins ties, else storeys_from_permits.py BP-description demo-guarded; FEET beats storeys×3.5 where stated); FOOTPRINTS = me (assessor via project_parcels→berkeley.db). Convention 3.5 m/storey.**
...
**PLACEHOLDER FINDINGS — the gap that matters is ZERO.** 65 placeholders lack a resolved height on MY side: **6 built**, **47 small ADU/infill 0-2u**, **16 proposed 3+u no-CO**. The 2 camera-visible ones I'd flagged as unresolved are **RESOLVED by 2b** — proj51 2942 College = 2 st (permit "construct new TWO-STORY... four dwelling units") and proj59 1514 Walnut = 3 st ("new THREE STORY... SB 684"): the story counts live in the PLANNING-record description, which 2b's `storeys_from_permits.py` reads but my `resolve_placeholders.py` (permits-table only) missed — a lesson: read the Planning description, not just `permits`. proj91 2009 Addison = the only one of the 65 with a 1.E on R2, but it's IMAGE-ONLY (0 extractable text) — unparseable, already resolved via Overture 7st. CONCLUSION: every building the flyby dwells on has a sourced height; the remainder legitimately holds.

**CORRECTIONS from 2b (post my snapshot, all on dev — f654fce/b26b561/4df61b2/e754535/c6b622e):**
- **geometry.kml = 196 placemarks, stamp `geom-e112113faf25`** (not 198). Dup-placemark deletions DONE (2099 MLK survivor = Completed/72u raised to 24.5m; 2740 Shasta In-Review survivor) — NOT pending. Each dup held a different correct field, so neither was a plain delete.
- **FEET beats storeys×3.5 where a 1.E states a height in feet** — the measured figure wins; 3.5 m/storey is ONLY the fallback for a storey count with no stated feet. 2190 Shattuck = 360.8 ft = **110.0 m**, NOT 34×3.5=119 m. `stated_heights.csv` carries both `stories_proposed` + `height_ft_proposed`; use `height_ft_proposed × 0.3048` where present. (2b's 1.E-precedence sweep b26b561 corrected 5 geometry.kml heights, worst 2720 San Pablo 35.7→20.1 m, which had been inflating the San Pablo corridor.)
- **tours.json / served catalog was 404-ing EVERY tour download** (catalog pointed at a geom generation absent on disk). Root cause structural: every rebuild changes the geom sha → every package filename → catalog re-orphans. FIXED c6b622e: `build_tour_package.py --all` now repoints the catalog + verifies every entry resolves on disk BEFORE writing (non-zero exit on a missing generation). **Correction to my earlier note: `--all` does NOT silently keep tours.json in sync — the guard does.**

**OPEN for John (gated):** 2205/2201 Blake footprints stay unpromoted until the 13,288 sf double lot is split geometrically (2b holds them; the co-located pair is deliberate, the only one left). Snapshots this session: `keep_snapshot_2026-08-25_pre-{tower,camera,major,bart-classification}-ingest.db`. **Bond-map Chronicle links now copy the address to clipboard on click (aff877f, all 3 maps) — awaiting John's deploy.**

## 2026-09-02 [finance-curriculum + jn-l] — 🎓 FIN curriculum seeded; JN-L re-baselined post-refresh; participant-role answers for tours lane
**Curriculum:** `notebooks/finance_curriculum/` seeded (`f5cdc54`+`47dadb1`): FIN1 built/executed/gate-PASS (revenue+spend Sankeys from the budget external-facts JSON; Colab-safe — GitHub-raw fallback), README maps FIN2–6 to existing analyses. **Colab links:** FIN1 runnable via Drive (colab.research.google.com/drive/1tc0oJSf4b5Iv2rNkLz3nAIk947W7LUvA — John must set link-sharing before mailing); JN-L viewable via github/main link, NOT runnable (local DBs).
**JN-L (`a5e64ed`):** Colab guard added (stops cleanly outside repo, points to FIN1). Gate had correctly HALTED on legit drift (2026-08-21 assessor refresh = 2026-27 roll enrolled ~$104M more construction; tracked_imps 335.9→439.8, benchmark 432→441, **enrolled-now floor $441M→$545M = $1.77M/yr City GF**; v2 drift = Aug ingests + owner join) → APPENDED `fiscal_flows_baseline_2026-09-02.json`, re-executed, PASS.
**ANSWERS for tours/site lane (asked 2026-09-02, their session restarted — sent to berkeley-data-d7, durable copy here):** CPRA BP_Annual xlsx has NO applicant/professional columns (headers verified — permit/dates/parcel/address/valuation/desc/ADU/worktype/units only). Accela: capID discovery = `discover_planning_records.py` + `url_discovery_scraper.py`; CapDetail plain-HTML parse of Owner/Licensed Professional is the cheap path; 1.E forms name the APPLICANT — extract in the harvest pass already running. Lenders/equity: CTCAC staff reports name construction lender + equity investor per LIHTC project (affordable subset); HCD award lists (already JN-L-queued); market-rate = recorder deeds of trust (fee) or press-with-provenance, version-scoped. **CTCAC LIST DOWNLOADED 2026-09-02: `data/raw/ctcac/CTCAC_List_of_Projects_2026-09-02.xlsx`** (treasurer.ca.gov/ctcac/projects; per-county sheets, 'Alameda' header row 0; **46 Berkeley rows** with Application Number, 9%/4%, Construction Type, DEVELOPER, Housing Type, address, **APN** — joinable via to_canonical_apn — Application Stage, PIS date). Application Number keys into CTCAC meeting e-binder staff reports for lender+investor. (Tried to message tours lane twice; their session keeps restarting — this entry is the durable handoff.)
**⚠ PUSH CAUTION (standing until resolved):** dev carries the Wozniak withdrawal letter, rebuttal analysis, McGrath memo — repo is PUBLIC; any `push dev:main` publishes them. Hold push until John resolves the Gordon matter or relocates the sensitive notes.
