#!/usr/bin/env python3
"""
Does-not-complete permits sample field dump
===========================================
Dumps every column of 10 sample permits linked to does_not_complete_project
CO events (first 10 by permit_id).
...
OUTPUT_FILE = Path(__file__).parent / 'does_not_complete_sample_full_fields.txt'
...
def main():
    ...
    co_events = cur.execute("""
        ...
        ORDER BY pe.permit_id
    """).fetchall()

    # Find does_not_complete_project permits (take first 10)
    does_not_complete_permit_ids = []
    for row in co_events:
        text = row['permit_description'] or row['event_summary']
        if classify_permit_for_completion(text) == 'does_not_complete_project':
            does_not_complete_permit_ids.append(row['permit_id'])
            if len(does_not_complete_permit_ids) >= 10:
                break
    ...
    lines = []
    lines.append("=" * 80)
    lines.append("DOES_NOT_COMPLETE_PROJECT PERMITS - SAMPLE FIELD DUMP (COUNTER-CONTROL)")
    lines.append(f"Sample size: {len(does_not_complete_permit_ids)} (first 10 by permit_id)")
    lines.append("=" * 80)

    for permit_id in does_not_complete_permit_ids:
    ...
    print(f"Written {len(does_not_complete_permit_ids)} permits to: {OUTPUT_FILE}")
