...
def build(tour_path: pathlib.Path) -> pathlib.Path:
    ...
    # AFTER the Document's own <name>, not before it. KML requires a Feature's name ahead of
    # its child features, and this used to inject straight after the <Document> tag, so every
    # package in the library came out as
    #     <Document><gx:Tour>...</gx:Tour><name>...</name><Style>...
    # Earth still DRAWS that -- which is why it went unnoticed for months -- but Movie Maker
    # would not offer the tour for recording (John, 2026-08-30: "I can't use moviemaker yet on
    # this kmz"). minidom.parseString below checks well-formedness, not element ORDER, so the
    # gate had nothing to say about it.
    ...
    name_end = re.compile(r"</name>").search(out, doc_tag.end())
    at = name_end.end() if name_end else doc_tag.end()
    out = out[:at] + stamp + tour + out[at:]

    minidom.parseString(out)  # well-formedness gate — refuse to emit broken XML
    # ORDER gate. Well-formed is not the same as valid: the tour must follow the Document's
    # <name>, or Movie Maker will not list it. Assert what we just relied on.
    _d = out.find("<Document")
    if not (_d < out.find("</name>", _d) < out.find("<gx:Tour")):
        raise SystemExit(f"{tour_path.name}: <gx:Tour> precedes the Document <name> — "
                         f"Movie Maker will not see it")
